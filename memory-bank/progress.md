...
## What Works
- `samples/Tailwind.Avalonia.Sample/SampleShell.axaml.cs` now separates section browsing from page navigation, so changing sections only refreshes the page list UI and leaves the currently displayed sample page untouched until a page row is clicked.
- `samples/Tailwind.Avalonia.Sample/SampleShell.axaml` now behaves more like a GitHub-style navigation drawer: the pane starts closed, opens as an overlay instead of consuming layout width, includes a close icon in-pane, and closes again after navigation while the flatter top bar keeps more vertical room for page content.
- `samples/Tailwind.Avalonia.Sample/SampleShell.axaml` now keeps the content header compact by presenting section and page context on one line with reduced vertical padding and typography, so the header height reads much closer to the hamburger toggle chrome.
