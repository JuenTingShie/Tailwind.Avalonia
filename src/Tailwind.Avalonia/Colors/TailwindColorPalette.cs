...
internal static class TailwindColorPalette
{
    private static readonly Regex ColorDeclarationPattern = new(
    ...
    private static IReadOnlyList<TailwindColorToken> LoadTokens()
    {
        ...
        foreach (Match match in ColorDeclarationPattern.Matches(PaletteCss))
        {
            var name = match.Groups["name"].Value;
            var value = match.Groups["value"].Value;

            results.Add(new TailwindColorToken(ToResourceSuffix(name), TailwindCssColorParser.Parse(value)));
        }

        if (results.Count == 0)
        {
            throw new InvalidOperationException("No Tailwind color tokens were loaded from the official palette source.");
        }
    ...
    private static string ToResourceSuffix(string tokenName)
    {
        ...
        foreach (var segment in tokenName.Split('-', StringSplitOptions.RemoveEmptyEntries))
        {
    ...
    internal readonly record struct TailwindColorToken(string ResourceSuffix, Color Color);

    // Official Tailwind CSS v4.2 default palette from the docs/default palette reference.
    private const string PaletteCss = """
